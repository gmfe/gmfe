...
const Guide = styled.div`
  ...
  transform: ${props => {
    ...
    const autoPosition = coords => {
      const positionsOrder = util.bestPositionOf(available)
    ...
    const pos = helperPosition => {
      if (Array.isArray(helperPosition)) {
        const isOutX = util.isOutsideX(helperPosition[0], windowWidth)
        const isOutY = util.isOutsideY(helperPosition[1], windowHeight)
        const warn = (axis, num) => {
          console.warn(
      ...
      const hX = util.isOutsideX(targetLeft + helperWidth, windowWidth)
        ? util.isOutsideX(targetRight + padding, windowWidth)
          ? targetRight - helperWidth
          : targetRight - helperWidth + padding
        : targetLeft - padding
      const x = hX > padding ? hX : padding
      const hY = util.isOutsideY(targetTop + helperHeight, windowHeight)
        ? util.isOutsideY(targetBottom + padding, windowHeight)
