...
import { Form, FormItem, FormButton, FormBlock, FormGroup, FormPanel } from './index'
import {
  Select,
  Switch,
  Validator,
  ...
  return (
    <FormBlock col={2}>
      ...
      <FormItem label='地区地区地区'>
        <Select data={area} value={store.area} onChange={(value) => store.setData('area', value)} />
      </FormItem>
      <FormItem label='是否工作' required>
        <Switch checked={store.isWork} onChange={(checked) => store.setData('isWork', checked)} />
      </FormItem>
      <FormItem label='描述' required validate={Validator.create([], store.desc)}>
        <textarea value={store.desc} onChange={(e) => store.setData('desc', e.target.value)} />
      </FormItem>
    </FormBlock>
  )
})
    ...
    () => (
      <Form disabledCol labelWidth='100px' onSubmit={() => console.log('onSubmit')}>
        ...
        <FormItem label='自定义' unLabelTop>
          <div>
            这里是自定义。没有上边距的情况下，label 要对齐，则提供 unLabelTop 去掉 label 的上边距
          </div>
        </FormItem>
        <FormItem label='身高'>
          <InputNumberV2
              ...
              <FormItem
                ...
                tooltip={
                  <div className='gm-padding-10 gm-bg' style={{ width: '455px' }}>
                    <p style={{ marginBottom: '4px', fontSize: '12px' }}>23333333333333333</p>
